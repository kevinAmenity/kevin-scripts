...
def _print_line(line, stage_num, space_char):
...
def read_results(document):
    ...
    if not os.path.isfile(document) or os.path.splitext(document)[1] != '.csv':
...
def remove_scoring_section(data):
...
def create_dict_with_event_type(data):
...
def create_df_for_each_event_type(dict_data):
    ...
    return {et: pd.DataFrame(columns=data[0] + ['Event Type'], 
                             data=[x + [et] for x in data[1:]]).drop(columns=[''])
...
def _rename_duplicate_columns(df):
    ...
    for dup in cols[cols.duplicated()].unique():
        cols[cols[cols == dup].index.values.tolist()] = ['{}_{}'.format(dup, str(idx))
                                                        if idx != 0 else dup 
...
def remove_dashes_from_sentence_start(data):
...
def pull_other_entity_values(data, option):
...
def reorder_columns(data):
    ...
    data = data[good_cols[:split_num] + 
               [name for name in data if name not in good_cols] + 
               good_cols[split_num:]]
    return data

    
def write_document(data, document):
    ...
    document = '{}_final.csv'.format(document[:-4])
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Converts the results CSV '
                                     'file into a more user-friendly CSV file.')
    parser.add_argument('--document', required=True, help='The results '
                        'document to convert (.csv).')
    parser.add_argument('--pullothervalues', required=False, action='store_true', 
                        help='Pull slot values from "Other" column to their '
                        'respective columns (defualt FALSE).')
    ...
    data = read_results(document)                            # read data from document
    data = remove_scoring_section(data)                      # remove the top scoring data
    data = create_dict_with_event_type(data)                 # break results into chunks corresponding to event type
    data = create_df_for_each_event_type(data)               # convert from array to pandas DataFrame for each event type
    data = merge_dataframes(data)                            # merge all individual DataFrames into one
    data = add_neutral_polarity(data)                        # adds 'NEUTRAL' to the 'Polarity' column
    data = remove_dashes_from_sentence_start(data)           # removes "-"s from the beginning of sentences
    data = pull_other_entity_values(data, pull_others)       # pull slot values from 'Other' column
    data = delete_blank_columns(data)                        # delete slot columns containing no values
    data = reorder_columns(data)                             # reorders columns
    write_document(data, document)                           # write final document
    
    print('\r\nAll done; finished in {} seconds.'.format(str(round(time.perf_counter() - start_time, 2))))